...
The **table view** lists everything screened, rejects included. Click a symbol for its
chart, criteria and hypothetical plan; filter by setup, by any combination of statuses,
and by eligibility; drag the slicers to band score and RS; and click the Grade, Score, RS
or Mkt cap header to sort (best first, then reversed, then back to the engine's own
ranking). Rows with no value for the sorted column stay at the bottom either way.
